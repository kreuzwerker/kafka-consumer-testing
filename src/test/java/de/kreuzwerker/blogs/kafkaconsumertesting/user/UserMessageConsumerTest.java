...
import de.kreuzwerker.blogs.kafkaconsumertesting.messaging.MockAvroDeserializer;
...
class UserMessageConsumerTest extends PostgresAndKafkaTestCase {
  ...
  void testCreateUser() throws ExecutionException, InterruptedException {
    ...
    String userId = UUID.randomUUID().toString();
      UserEvent event = UserEvent.newBuilder().setId(userId).setFirstName("Jane").setLastName("Doe").build();

      List<Header> headers =
              singletonList(new RecordHeader(EVENT_TYPE_KEY, EVENT_CREATE.getBytes()));
      ProducerRecord record =
              new ProducerRecord<>(
                      MockAvroDeserializer.USERS_TOPIC, null, event.getId(), event, headers);

      sendEvent(producer, record);

      ConsumerRecord<String, UserEvent> singleRecord =
              KafkaTestUtils.getSingleRecord(consumer, MockAvroDeserializer.USERS_TOPIC);
    ...
    void prepare() {
        ...
        consumer.subscribe(Collections.singleton(MockAvroDeserializer.USERS_TOPIC));
